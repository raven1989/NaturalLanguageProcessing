...
namespace Darts {
  ...
  private:
    ...
    // fetch输入一个节点，构造好该节点的所有字节点在siblings中，是广度的
    size_t fetch(const node_t &parent, std::vector <node_t> &siblings) {
      ...
      for (size_t i = parent.left; i < parent.right; ++i) {
        // length 是一个额外提供的数组，用于提供词的长度，默认为空，
        // 则使用Length<node_type_> operator(node_type_* key)计算长度：数到0
        // utf-8编码就不适合用默认的
        // 如果一个词的长度小于父亲的层数，说明这个词已经在trie树中分配完了
        ...
        array_u_type_ cur = 0;
        if ((length_ ? length_[i] : length_func_()(key_[i])) != parent.depth)
          // 字的code为什么要+1？因为0被用作root节点的code，为了让出0,所有字的code+1
          cur = (array_u_type_)tmp[parent.depth] + 1;

        // 要求词典按照字典序排序，所以如果前一个输入比现在的大，说明不是字典序，故而出错结束
        if (prev > cur) {
          error_ = -3;
          return 0;
        }
        ...
        if (cur != prev || siblings.empty()) {
          ...
          tmp_node.code  = cur;
          tmp_node.left  = i;
          // 添加了新的兄弟节点，如果前面有兄弟节点，次数词数又产生了一个分割，
          // 所以将前一个兄弟索引的结束词位置写好
